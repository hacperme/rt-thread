...
void test_settings() {
    ...
    if(settings_init(&settings, "/settings.conf", NULL) != 0) {
        log_error("settings_init error.");
        return;
    }

    log_debug("settings_init success.");

    pParams = settings_read(&settings);
    if(!pParams) {
        log_error("settings_read error.");
        return;
    }

    log_info("read parmas: %d", pParams->collect_interval);

    params.collect_interval = 100;
    if(settings_update(&settings, &params) != 0) {
        log_error("settings_update error.");
        return;
    }

    log_debug("settings_update success.");
    ...
    log_debug("re settings_init success.");
