/*
 * @FilePath: at_client_http.c
 * @Author: Ye Jian
 * @brief     : <Description>
 * @version   : v1.0.0
 * @Date: 2024-08-30 13:24:33
...
#define PRINTF_RESP(X) do{	\
	if(X && X->buf){		\
		log_info("resp_buf-> lene:%d : data:%s\n",__LINE__, X->buf);	\
	}	\
}while(0);
...
void cat1_at_client_init(void)
{
	...
	{
		log_error("AT Client1 (uart1) initialize failed\n");
	}
    ...
    log_info("AT Client1 (uart1) initialized success\n");
...
int at_http_upload_file_chunked(const char *filename)
{
	...
	if (!resp) {
        log_error("Failed to create AT response object\n");
        return -1;
    }
	send_resp = at_create_resp(RESP_BUFFER_SIZE, 1, 3000);
	if (!send_resp) {
        log_error("Failed to create AT send response object\n");
        return -1;
    }
    ...
    if (at_obj_exec_cmd(client, resp, "AT+QIACT?") < 0) {
        log_error("Failed to execute AT+QIACT?\n");
        goto err;
    }
	...
	if (at_resp_parse_line_args(resp, 2, "+QIACT: %*d,%d", &pdp_status) <= 0) {
        log_error("Failed to parse AT+QIACT? response\n");
		pdp_status = 0;
    }

	PRINTF_RESP(resp);
	log_error("pdp status is %d\n", pdp_status);

	
    if (pdp_status != 1) {
        log_error("PDP context is not active, cannot proceed\n");
		pdp_status = 0;
        //return;
    }
...
//        log_error("Failed to open TCP connection, status: %d\n", pdp_status);
//		PRINTF_RESP(resp);
//        at_delete_resp(resp);
//        return;
//    }
	...
	{
		log_error("QIURC no urc, timeout.\n");
		goto err;
	}
    ...
    if (at_obj_exec_cmd(client,resp, "AT+QIRD=1") < 0) {
        log_error("Failed to read server response\n");
    } else {
...
//    }
    ...
    log_error("File 'felix.txt' created and content written successfully.\n");
...
rt_err_t cat1_qpowd()
{
    ...
    if (resp == RT_NULL) {
        log_debug("no enought mem for cat1 at resp");
        return RT_ERROR;
    }
...
rt_err_t cat1_check_network(int retry_times)
{
    ...
    for (int i=0; i < retry_times; i++) {
        ...
        log_debug("cat1 check status, n: %d, stat: %d", n, stat);
        if (stat == 1 || stat == 5) {
            at_delete_resp(resp);
            return RT_EOK;
        }
...
rt_err_t cat1_enable_echo(int enable)
{
    ...
    if (result != RT_EOK) {
        log_error("cat1 disable at echo failed: %d", result);
    }
...
rt_err_t cat1_set_network_config()
{
    ...
    if (result != RT_EOK) {
        log_debug("at_obj_exec_cmd AT+QEREG=2 failed");
        goto ERROR;
    }
    ...
    if (result != RT_EOK) {
        log_error("at_obj_exec_cmd AT+QIACT?: %d", result);
        goto ERROR;
    }
    log_debug("set AT+QIACT? success");

    result = at_obj_exec_cmd(client, resp, "AT+QIACT=1");
    if (result != RT_EOK) {
        log_error("at_obj_exec_cmd AT+QIACT=1: %d", result);
        goto ERROR;
    }
    log_debug("set AT+QIACT=1 success");
