...
static void feed_rtwdg(void)
{
    ...
    log_debug("RT_DEVICE_CTRL_WDT_KEEPALIVE %s", RT_RES_MSG(res));
}
...
static rt_err_t test_wdg(void)
{
    ...
    log_debug("Find %s device %s.", WDT_DEVICE_NAME, RT_RES_MSG(res));
    if (res == RT_ERROR)
    {
        return res;
    }
    ...
    // log_debug("rt_device_init wdg %s", RT_RES_MSG(res));
    // if (res == RT_ERROR)
    // {
    //     return res;
    // }
    ...
    // log_debug("RT_DEVICE_CTRL_WDT_GET_TIMEOUT %d %s.", timeout, RT_RES_MSG(res));
    // if (res != RT_EOK)
    // {
    //     return RT_ERROR;
    // }
    ...
    // log_debug("RT_DEVICE_CTRL_WDT_SET_TIMEOUT %d %s.", timeout, RT_RES_MSG(res));
    // if (res != RT_EOK)
    // {
    //     return RT_ERROR;
    // }
    ...
    log_debug("rt_thread_init WDG_THD %s", res == RT_EOK ? "success" : "failed");
    if (res != RT_EOK)
    {
        rt_err_t _res = rt_device_close(wdg_dev);
        log_debug("WDG closed %s.", RT_RES_MSG(_res));
        return res;
    }

    res = rt_thread_startup(&WDG_THD);
    log_debug("wdg_feeded start %s", res == RT_EOK ? "success" : "failed");
    if (res != RT_EOK)
    {
        rt_err_t _res = rt_device_close(wdg_dev);
        log_debug("WDG closed %s.", RT_RES_MSG(_res));
    ...
    log_debug("RT_DEVICE_CTRL_WDT_START %s.", RT_RES_MSG(res));
...
{
    log_debug("__HAL_RCC_GET_FLAG(RCC_FLAG_BORRST)=%d", __HAL_RCC_GET_FLAG(RCC_FLAG_BORRST));
    log_debug("__HAL_RCC_GET_FLAG(RCC_FLAG_OBLRST)=%d", __HAL_RCC_GET_FLAG(RCC_FLAG_OBLRST));
    log_debug("__HAL_RCC_GET_FLAG(RCC_FLAG_PINRST)=%d", __HAL_RCC_GET_FLAG(RCC_FLAG_PINRST));
    log_debug("__HAL_RCC_GET_FLAG(RCC_FLAG_SFTRST)=%d", __HAL_RCC_GET_FLAG(RCC_FLAG_SFTRST));
    log_debug("__HAL_RCC_GET_FLAG(RCC_FLAG_IWDGRST)=%d", __HAL_RCC_GET_FLAG(RCC_FLAG_IWDGRST));
    log_debug("__HAL_RCC_GET_FLAG(RCC_FLAG_WWDGRST)=%d", __HAL_RCC_GET_FLAG(RCC_FLAG_WWDGRST));
    log_debug("__HAL_RCC_GET_FLAG(RCC_FLAG_LPWRRST)=%d", __HAL_RCC_GET_FLAG(RCC_FLAG_LPWRRST));
...
static void test_feed_wdg_soft(void *args)
{
    ...
    if (*params == 0)
    {
        ...
        log_debug("wdg_create_soft %s wdg_id=%d.", RT_RES_MSG(res), wdg_id);
        if (res != WDG_NO_ERROR)
        {
            return;
        }
        while (cnt > 0)
        {
            ...
            log_debug("wdg_feed_soft(%d) %s.", wdg_id, RT_RES_MSG(res));
    ...
    else if (*params == 1)
    {
        ...
        while (cnt > 0)
        {
            ...
            if (recv_size > 0)
            {
                log_debug("rt_mq_recv msg_size=%d, msg=%s", recv_size, recv_msg);
            }
            res = wdg_feed_soft(wdg_id);
            log_debug("wdg_feed_soft(%d) %s.", wdg_id, RT_RES_MSG(res));
            cnt--;
    ...
    log_debug("wdg_destroy_soft(wdg_id) %s.", RT_RES_MSG(res));
    // shut_down();
    RT_ASSERT(res != 0);
}

rt_err_t test_watch_dog(void)
{
    rt_err_t res;

    res = rt_mq_init(&wdgq, "wdgq", wdgq_pool, wdgq_msg_size, wdgq_pool_size, RT_IPC_FLAG_PRIO);
    log_debug("rt_mq_init wdgq %s", RT_RES_MSG(res));
    if (res != RT_EOK)
    {
        return res;
    }
    ...
    log_debug("wdg_init %s.", RT_RES_MSG(err));
    ...
    log_debug("rt_thread_create testwdg %s.", RT_RES_MSG(res));
    if (res == RT_ERROR)
    {
        return res;
    }
    ...
    log_debug("rt_thread_startup testwdg %s.", RT_RES_MSG(res));
