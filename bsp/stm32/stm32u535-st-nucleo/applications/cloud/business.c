...
{
    // LOG_D("adxl372_inact_event_handler called");
...
int record_wakeup_source()
{
    ...
    LOG_D("wakeup souce flag: %d", wakeup_source_flag);
    return 0;
}

int external_devices_init()
{
    LOG_D("external devices init");
    rt_err_t res = RT_EOK;
    ...
    res = adxl372_init();
    LOG_D("adxl372_init %s", res != RT_EOK ? "failed" : "success");
...
int collect_sensor_data()
{
    ...
    if (wakeup_source_flag != RTC_SOURCE) {
        ...
        LOG_D("adxl372_measure_acc %s", read_acc_xyz_result == RT_EOK ? "success" : "failed");
    ...
    LOG_D(msg);
    // LOG_D("temp116_read_temperture %s, temp1=%f", res != RT_EOK ? "failed" : "success", temp1);
    ...
    LOG_D(msg);
    // LOG_D("temp116_read_temperture %s, temp2=%f", res != RT_EOK ? "failed" : "success", temp2);
    if (res == RT_EOK) {
    ...
    LOG_D(msg);
    // LOG_D("hdc3021_read_temp_humi %s, temp3=%f, humi=%f", res != RT_EOK ? "failed" : "success", temp3, humi);
    if (res == RT_EOK) {
    ...
    if (wakeup_source_flag != RTC_SOURCE) {
        ...
        for (int i=0; i < 30; i++) {
            ...
            if (nmea_item.is_valid) {
                ...
                LOG_D("nmea_item->GNRMC: %s", nmea_item.GNRMC);
                LOG_D("nmea_item->is_valid: %d", nmea_item.is_valid);
                LOG_D("nmea_item->GNGGA: %s", nmea_item.GNGGA);
                LOG_D("nmea_item->GNGLL: %s", nmea_item.GNGLL);
                LOG_D("nmea_item->GNVTG: %s", nmea_item.GNVTG);
                LOG_D("nmea_item->GNGSA_SIZE: %d", nmea_item.GNGSA_SIZE);
                break;
            }
...
enum nbiot_network_status nbiot_wait_network_ready()
{
    if (nbiot_wait_network_retry_times >= 3) {
        LOG_D("nbiot_wait_network_retry_times >= 3, goto sleep");
        return NBIOT_NETWORK_NOT_RDY;
    }
    nbiot_wait_network_retry_times += 1;

    LOG_D("nbiot wait network ready");

    nbiot_enable_echo(0);
    if (nbiot_check_network(10) == RT_EOK) {
        LOG_D("nbiot network has been ready");
        rt_err_t result = RT_EOK;
        result = nbiot_get_current_datetime(&current_time);
        LOG_D("current_time.year: %d", current_time.year);
        LOG_D("current_time.month: %d", current_time.month);
        LOG_D("current_time.day: %d", current_time.day);
        LOG_D("current_time.hour: %d", current_time.hour);
        LOG_D("current_time.minute: %d", current_time.minute);
        ...
        if (! set_network_config_flag) {
            ...
            if(nbiot_set_network_config(&config) == RT_EOK) {
                ...
                LOG_D("nbiot_set_network_config failed");
            }
        }
...
int nbiot_wait_server_connect_ready()
{
    if (nbiot_wait_server_connect_retry_times >= 5) {
        LOG_D("nbiot_wait_server_connect_retry_times >= 5, goto sleep");
        return NBIOT_SERVER_CONNECT_NOT_RDY;
    }
    nbiot_wait_server_connect_retry_times += 1;

    LOG_D("nbiot wait server connect");
    rt_err_t result = RT_EOK;
    ...
    if (nbiot_check_qiotstate() == RT_EOK) {
        ...
        if (! set_lwm2m_config_flag && nbiot_check_lwm2m_config(&config) != RT_EOK) {
            if (nbiot_set_lwm2m_config(&config) == RT_EOK) {
                ...
                LOG_D("nbiot_set_lwm2m_config success");
            }
            else {
                LOG_D("nbiot_set_lwm2m_config failed");
            }
        }
...
int nbiot_report_ctrl_data_to_server()
{
    if (nbiot_report_ctrl_retry_times >= 3) {
        LOG_D("nbiot_report_ctrl_retry_times >= 3, goto report sensor data");
    ...
    if (nbiot_report_ctrl_data(data, rt_strlen(data)) == RT_EOK) {
        ...
        LOG_D("report ctrl data to server falied");
        if (nbiot_check_qiotstate() == RT_EOK) {
            return NBIOT_REPORT_CTRL_DATA_RETRY;
        }
        else {
            LOG_D("lwm2m server connect error, goto wait server connect");
            return NBIOT_SERVER_CONNECT_RETRY;
        }
    }
}
...
int nbiot_report_sensor_data_to_server()
{
    if (nbiot_report_sensor_data_retry_times >= 3) {
        LOG_D("nbiot_report_sensor_data_retry_times >= 3, goto cat1 upload file");
        return NBIOT_REPORT_SENSOR_DATA_FAILED;
    }
    nbiot_report_sensor_data_retry_times += 1;
    
    LOG_D("nbiot report sensor data to server");

    if (rt_strlen(nmea_buf) > 0) {
        LOG_D("send nmea_buf data!");
        nbiot_set_qiotlocext(nmea_buf);
    }
    else {
        LOG_D("nmea_buf char is NULL");
    }
...
enum cat1_network_status cat1_wait_network_ready()
{
    if (cat1_wait_network_retry_times >= 3) {
        LOG_D("cat1_wait_network_retry_times >= 3, goto sleep");
...
int cat1_upload_file()
{
    if (cat1_upload_file_retry_times >= 3) {
        LOG_D("cat1_upload_file_retry_times >= 3, goto sleep");
...
void stm32_sleep()
{   
    LOG_D("wait stm32_sleep_ack_sem release");
