...
rt_err_t adxl372_enable_inactive_irq(rt_uint16_t *milliseconds, rt_uint16_t *threshold)
{
    ...
    LOG_D("adxl372_set_standby %s", res == RT_EOK ? "success" : "failed");
    if (res != RT_EOK)
    {
        return res;
    }
    ...
    LOG_D("adxl372_full_bandwidth_measurement_mode %s", res == RT_EOK ? "success" : "failed");
...
rt_err_t adxl372_disable_inactive_irq(void)
{
    ...
    LOG_D("adxl372_recv_inact_event_thd_stop %s", res == RT_EOK ? "success" : "failed");
    if (res != RT_EOK)
    {
        return res;
    }
    ...
    LOG_D("adxl372_int1_pin_irq_disable %s", res == RT_EOK ? "success" : "failed");
    if (res != RT_EOK)
    {
        return res;
    }
...
rt_err_t adxl372_query_xyz(adxl372_xyz_t xyz)
{
    ...
    if (res == RT_EOK)
    {
        // LOG_D("X VAL %d, Y VAL %d, Z VAL %d", XYZ_VAL[0], XYZ_VAL[1], XYZ_VAL[2]);
...
rt_err_t adxl372_set_int1_map(rt_uint8_t *val)
{
    ...
    LOG_D("adxl372_set_int1_map reg=0x%02X val=0x%02X reread=0x%02X res=%d", ADI_ADXL372_INT1_MAP, *val, recv_buf, res);
    res = (recv_buf == *val ? RT_EOK : RT_ERROR);
    return res;
}
...
rt_err_t adxl372_set_hpf(rt_uint8_t *val)
{
    ...
    LOG_D("adxl372_set_hpf reg=0x%02X val=0x%02X reread=0x%02X res=%d", ADI_ADXL372_HPF, *val, recv_buf, res);
    res = (recv_buf == *val ? RT_EOK : RT_ERROR);
    return res;
}
...
rt_err_t adxl372_reset(void)
{
    ...
    rt_uint8_t recv_buf = 0xFF;
    res = adxl732_read(ADI_ADXL372_TIMING, &recv_buf, 1);
    LOG_D("adxl372_reset reg=0x%02X val=0x%02X reread=0x%02X, res=%d", ADI_ADXL372_TIMING, val, recv_buf, res);
    res = (recv_buf == val ? RT_EOK : RT_ERROR);
    return res;
}
...
static void test_adxl372(int argc, char **argv)
{
    ...
    {
        res = adxl372_reset();
        LOG_D("adxl372_reset %s", res != RT_EOK ? "failed" : "success");
    }
    ...
    if (run_always == 1)
    {
        ...
        while (1)
        {
            ...
            {
                LOG_D("adxl372_query_xyz failed.");
            }
