...
rt_err_t test_nbiot_send_at_cmd(int argc, char **argv)
{
    ...
    if (result != RT_EOK) {
        LOG_E("nbiot_send_at_cmd failed: %d", result);
    }
    for (int i=1; i <= resp->line_counts; i++) {
        LOG_D("recv line %d: %s", i, at_resp_get_line(resp, i));
    }
...
void test_nbiot_get_current_datetime()
{
    ...
    LOG_D("nbiot_get_current_datetime result: %d", result);
    if (result == RT_EOK) {
        LOG_D("year: %d", dt.year);
        LOG_D("month: %d", dt.month);
        LOG_D("day: %d", dt.day);
        LOG_D("hour: %d", dt.hour);
        LOG_D("minute: %d", dt.minute);
        LOG_D("second: %d", dt.second);
        LOG_D("zone: %d", dt.zone);
    }
...
rt_err_t test_cat1_send_at_cmd(int argc, char **argv)
{
    ...
    result = at_obj_exec_cmd(client, resp, argv[1]);
    if (result != RT_EOK) {
        LOG_E("cat1_send_at_cmd failed: %d", result);
    }
    for (int i=1; i <= resp->line_counts; i++) {
        LOG_D("recv line %d: %s", i, at_resp_get_line(resp, i));
    }
