#include "nbiot.h"
#include <stdio.h>
#include "cJSON.h"

#define DBG_TAG "nbiot"
#define DBG_LVL DBG_LOG
#include <rtdbg.h>

static struct rt_mutex qiot_event_mutex;
static int QIOT_SUBSCRIBE_EVENT_CODE = -1;
static int QIOT_CONNECT_EVENT_CODE = -1;
static int QIOT_AUTH_EVENT_CODE = -1;
static int QIOT_DATE_SEND_EVENT_CODE = -1;
static int QIOT_DATE_RECV_EVENT_CODE = -1;
static int QIOT_DISCONNECT_EVENT_CODE = -1;
static int QIOT_OTA_EVENT_CODE = -1;
static int QIOT_PLATFORM_EVENT_CODE = -1;
static int QIOT_WAKEUP_EVENT_CODE = -1;

static rt_mq_t cclk_data_urc_queue = RT_NULL;

void nbiot_qiotevt_urc_handler(struct at_client *client, const char *data, rt_size_t size)
{
    int event_type;
    int event_code;
    char event_data[256] = {0};

    // +QIOTEVT: <event_type>,<event_code>[,<data>]
    sscanf(data, "+QIOTEVT: %d,%d,%s", &event_type, &event_code, event_data);
    LOG_D("event_type: %d; event_code: %d; event_data: %s", event_type, event_code, event_data);
    
    switch (event_type)
    {
        case 1:
            rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
            QIOT_AUTH_EVENT_CODE = event_code;
            LOG_D("QIOT_AUTH_EVENT_CODE: %d", QIOT_AUTH_EVENT_CODE);
            rt_mutex_release(&qiot_event_mutex);
            break;
        case 2:
            rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
            QIOT_CONNECT_EVENT_CODE = event_code;
            LOG_D("QIOT_CONNECT_EVENT_CODE: %d", QIOT_CONNECT_EVENT_CODE);
            rt_mutex_release(&qiot_event_mutex);
            break;
        case 3:
            rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
            QIOT_SUBSCRIBE_EVENT_CODE = event_code;
            LOG_D("QIOT_SUBSCRIBE_EVENT_CODE: %d", QIOT_SUBSCRIBE_EVENT_CODE);
            rt_mutex_release(&qiot_event_mutex);
            break;
        case 4:
            rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
            QIOT_DATE_SEND_EVENT_CODE = event_code;
            LOG_D("QIOT_DATE_SEND_EVENT_CODE: %d", QIOT_DATE_SEND_EVENT_CODE);
            rt_mutex_release(&qiot_event_mutex);
            break;
        case 5:
            rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
            QIOT_DATE_RECV_EVENT_CODE = event_code;
            LOG_D("QIOT_DATE_RECV_EVENT_CODE: %d", QIOT_DATE_RECV_EVENT_CODE);
            rt_mutex_release(&qiot_event_mutex);
            break;
        case 6:
            rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
            QIOT_DISCONNECT_EVENT_CODE = event_code;
            LOG_D("QIOT_DISCONNECT_EVENT_CODE: %d", QIOT_DISCONNECT_EVENT_CODE);
            rt_mutex_release(&qiot_event_mutex);
            break;
        case 7:
            rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
            QIOT_OTA_EVENT_CODE = event_code;
            LOG_D("QIOT_OTA_EVENT_CODE: %d", QIOT_OTA_EVENT_CODE);
            rt_mutex_release(&qiot_event_mutex);
            break;
        case 8:
            rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
            QIOT_PLATFORM_EVENT_CODE = event_code;
            LOG_D("QIOT_PLATFORM_EVENT_CODE: %d", QIOT_PLATFORM_EVENT_CODE);
            rt_mutex_release(&qiot_event_mutex);
            break;
        case 9:
            rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
            QIOT_WAKEUP_EVENT_CODE = event_code;
            LOG_D("QIOT_WAKEUP_EVENT_CODE: %d", QIOT_WAKEUP_EVENT_CODE);
            rt_mutex_release(&qiot_event_mutex);
            break;
        default:
            break;
    }
}

void nbiot_cclk_urc_handler(struct at_client *client, const char *data, rt_size_t size)
{
    // +CCLK: 24/08/16,06:28:32+32
    LOG_E("nbiot_cclk_urc_handler called, size: %d; data: %s\n", size, data);
    rt_err_t result = rt_mq_send(cclk_data_urc_queue, data, size);
    if (result != RT_EOK) {
        LOG_E("nbiot_cclk_urc_handler rt_mq_send ERR\n");
    }
}

static struct at_urc nbiot_urc_table[] = {
    {"+QIOTEVT: ", "\r\n", nbiot_qiotevt_urc_handler},
    {"+CCLK: ", "\r\n", nbiot_cclk_urc_handler},
};

rt_err_t nbiot_at_client_init(void)
{
    rt_err_t result = RT_EOK;

    if (at_client_get(NBIOT_AT_UART_NAME) != RT_NULL) {
        LOG_E("nbiot at client already inited.\n");
        return RT_ERROR;
    }

    result = at_client_init(NBIOT_AT_UART_NAME, 256, 256);
    if (result != RT_EOK) {
        LOG_E("nbiot at client init failed: %d\n", result);
        return result;
    }
    LOG_D("nbiot at client init success.");

    result = at_obj_set_urc_table(at_client_get(NBIOT_AT_UART_NAME), nbiot_urc_table, 
                                  sizeof(nbiot_urc_table) / sizeof(nbiot_urc_table[0]));
    if (result != RT_EOK) {
        LOG_E("nbiot at set urc failed: %d\n", result);
        return result;
    }
    LOG_D("nbiot at set urc success.");

    result = rt_mutex_init(&qiot_event_mutex, "qiot_event_mutex", RT_IPC_FLAG_FIFO);
    if (result != RT_EOK) {
        LOG_E("qiot_event_mutex init failed: %d\n", result);
        return result;
    }
    LOG_D("qiot_event_mutex init success.");

    cclk_data_urc_queue = rt_mq_create("cclk_data_urc_queue", 64, 1, RT_IPC_FLAG_FIFO);
    if (cclk_data_urc_queue == RT_NULL) {
        LOG_E("cclk_data_urc_queue create failed!\n");
        return -RT_ERROR;
    }

    return result;
}
MSH_CMD_EXPORT(nbiot_at_client_init, nbiot_at_client_init);

rt_err_t nbiot_disable_sleep_mode()
{
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_obj_exec_cmd(client, RT_NULL, "AT");
    rt_thread_mdelay(200);

    at_response_t resp = at_create_resp(128, 0, rt_tick_from_millisecond(1000));
    if (resp == RT_NULL) {
        LOG_D("create resp failed.");
        result = RT_ERROR;
        goto ERROR;
    }

    result = at_obj_exec_cmd(client, resp, "AT+QSCLK=0");
    if (result == RT_EOK) {
        LOG_D("nbiot disable sleep mode ok");
    }
    else {
        LOG_D("nbiot disable sleep mode failed");
    }

ERROR:
    at_delete_resp(resp);
    return result;
}
MSH_CMD_EXPORT(nbiot_disable_sleep_mode, nbiot_disable_sleep_mode);

rt_err_t nbiot_enable_sleep_mode()
{
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_response_t resp = at_create_resp(128, 0, rt_tick_from_millisecond(5000));
    if (resp == RT_NULL) {
        LOG_D("create resp failed.");
        return RT_ERROR;
    }
    result = at_obj_exec_cmd(client, resp, "AT+QSCLK=1");
    if (result == RT_EOK) {
        LOG_D("nbiot enable sleep mode ok");
    }
    else {
        LOG_D("nbiot enable sleep mode failed");
    }

    at_delete_resp(resp);
    return result;
}
MSH_CMD_EXPORT(nbiot_enable_sleep_mode, nbiot_enable_sleep_mode);

rt_err_t nbiot_enable_echo(int enable)
{
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_response_t resp = at_create_resp(128, 0, rt_tick_from_millisecond(3000));
    if (resp == RT_NULL) {
        LOG_E("create resp failed.");
        return RT_ERROR;
    }

    // disable at echo
    result = at_obj_exec_cmd(client, resp, enable ? "ATE1" : "ATE0");
    if (result != RT_EOK) {
        LOG_E("disable at echo failed: %d", result);
    }

    at_delete_resp(resp);
    return result;
}

rt_err_t nbiot_check_lwm2m_config(lwm2m_config_t config)
{
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_response_t resp = at_create_resp(128, 0, rt_tick_from_millisecond(3000));
    if (resp == RT_NULL) {
        LOG_E("create resp failed.");
        return RT_ERROR;
    }

    // check production info
    char pk[64] = {0};
    char ps[64] = {0};
    char ver[64] = {0};
    result = at_obj_exec_cmd(client, resp, "AT+QIOTCFG=\"productinfo\"");
    if (result != RT_EOK) {
        LOG_E("check production info failed: %d", result);
        goto ERROR;
    }
    if (at_resp_parse_line_args(resp, 2, "+QIOTCFG: \"productinfo\",\"%[^\"]\",\"%[^\"]\",\"%[^\"]\"", pk, ps, ver) <= 0) {
        result = RT_ERROR;
        goto ERROR;
    }
    LOG_D("pk: %s; ps: %s; ver: %s", pk, ps, ver);
    if (rt_strcmp(pk, config->pk) != 0) {
        LOG_W("productinfo pk not equal to configure.");
        result = RT_ERROR;
        goto ERROR;
    }

    // check server
    int server_type = -1;
    char server_URL[128] = {0};
    result = at_obj_exec_cmd(client, resp, "AT+QIOTCFG=\"server\"");
    if (result != RT_EOK) {
        LOG_E("check server failed: %d", result);
        goto ERROR;
    }
    if (at_resp_parse_line_args(resp, 2, "+QIOTCFG: \"server\",%d,\"%[^\"]\"", &server_type, server_URL) <= 0) {
        result = RT_ERROR;
        goto ERROR;
    }
    LOG_D("server_type: %d; server_URL: %s", server_type, server_URL);
    if (rt_strcmp(server_URL, config->server_URL) != 0) {
        LOG_W("productinfo server not equal to configure.");
        result = RT_ERROR;
        goto ERROR;
    }

ERROR:
    at_delete_resp(resp);
    return result;
}

rt_err_t nbiot_set_lwm2m_config(lwm2m_config_t config)
{
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_response_t resp = at_create_resp(128, 0, rt_tick_from_millisecond(3000));
    if (resp == RT_NULL) {
        LOG_E("create resp failed.");
        return RT_ERROR;
    }

    // set productinfo
    result = at_obj_exec_cmd(client, resp, "AT+QIOTCFG=\"productinfo\",\"%s\",\"%s\"", config->pk, config->ps);
    if (result != RT_EOK) {
        LOG_E("set productinfo failed: %d", result);
        goto FINAL;
    }
    LOG_D("set productinfo success");

    // set server
    result = at_obj_exec_cmd(client, resp, "AT+QIOTCFG=\"server\",%d,\"%s\"", config->server_type, config->server_URL);
    if (result != RT_EOK) {
        LOG_E("set server failed: %d", result);
        goto FINAL;
    }
    LOG_D("set server success");

    // set lifetime
    result = at_obj_exec_cmd(client, resp, "AT+QIOTCFG=\"lifetime\",%d", config->lifetime);
    if (result != RT_EOK) {
        LOG_E("set lifetime failed: %d", result);
        goto FINAL;
    }
    LOG_D("set lifetime success");

    // set buffer
    result = at_obj_exec_cmd(client, resp, "AT+QIOTCFG=\"buffer\",%d", config->buffer_mode);
    if (result != RT_EOK) {
        LOG_E("set buffer failed: %d", result);
        goto FINAL;
    }
    LOG_D("set buffer success");

    // set act
    result = at_obj_exec_cmd(client, resp, "AT+QIOTCFG=\"act\",%d", config->context_id);
    if (result != RT_EOK) {
        LOG_E("set act failed: %d", result);
        goto FINAL;
    }
    LOG_D("set act success");

    // set tsl
    result = at_obj_exec_cmd(client, resp, "AT+QIOTCFG=\"tsl\",%d", config->tsl_mode);
    if (result != RT_EOK) {
        LOG_E("set tsl failed: %d", result);
        goto FINAL;
    }
    LOG_D("set tsl success");

FINAL:
    at_delete_resp(resp);
    return result;
}

rt_err_t nbiot_set_cfun_mode(int mode)
{
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_response_t resp = at_create_resp(128, 0, rt_tick_from_millisecond(3000));
    if (resp == RT_NULL) {
        LOG_D("create resp failed.");
        return RT_ERROR;
    }

    char s[20] = {0};
    snprintf(s, 20, "AT+CFUN=%d", mode);
    result = at_obj_exec_cmd(client, resp, s);
    if (result != RT_EOK) {
        LOG_E("nbiot cfun0 err: %d", result);
    }

    at_delete_resp(resp);
    return result;
}

rt_err_t nbiot_check_network(int retry_times)
{
    int n = -1;
    int stat = -1;
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_response_t resp = at_create_resp(128, 0, rt_tick_from_millisecond(3000));
    if (resp == RT_NULL) {
        LOG_D("create resp failed.");
        return RT_ERROR;
    }

    for (int i=0; i < retry_times; i++) {
        result = at_obj_exec_cmd(client, resp, "AT+CEREG?");
        at_resp_parse_line_args(resp, 2, "+CEREG: %d,%d", &n, &stat);
        LOG_D("check status, n: %d, stat: %d", n, stat);
        if (stat == 1 || stat == 5) {
            at_delete_resp(resp);
            return RT_EOK;
        }
        rt_thread_mdelay(5000);
    }

    at_delete_resp(resp);
    return RT_ERROR;
}


rt_err_t nbiot_set_network_config(network_config_t config)
{
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_response_t resp = at_create_resp(128, 0, rt_tick_from_millisecond(3000));
    if (resp == RT_NULL) {
        LOG_D("create resp failed.");
        return RT_ERROR;
    }

    char PDP_type[12] = {0};
    char APN[99] = {0};
    result = at_obj_exec_cmd(client, resp, "AT+QCGDEFCONT?");
    if (result != RT_EOK) {
        LOG_D("at_obj_exec_cmd AT+QCGDEFCONT=? failed");
        goto ERROR;
    }
    if (at_resp_parse_line_args(resp, 2, "+QCGDEFCONT: \"%[^,\"]\",\"%[^,\"]\",", PDP_type, APN) <= 0)
    {
        LOG_D("at_resp_parse_line_args AT+QCGDEFCONT? failed");
        goto ERROR;
    }
    LOG_D("read PDP_type: %s", PDP_type);
    LOG_D("read APN: %s", APN);
    if (rt_strcmp(APN, config->apn) != 0) {
        result = at_obj_exec_cmd(client, resp, "AT+QCGDEFCONT=\"IP\",\"%s\"", config->apn);
        if (result != RT_EOK) {
            LOG_E("set apn result: %d", result);
            goto ERROR;
        }
        LOG_D("set apn success");
    }

    char band_info[64] = {0};
    result = at_obj_exec_cmd(client, resp, "AT+QBAND?");
    if (result != RT_EOK) {
        LOG_D("at_obj_exec_cmd AT+QBAND? failed");
        goto ERROR;
    }
    if (at_resp_parse_line_args(resp, 2, "+QBAND: %s", band_info) <= 0) {
        LOG_D("at_resp_parse_line_args AT+AT+QBAND? failed");
        goto ERROR;
    }
    LOG_D("read band_info: %s", band_info);
    if (rt_strcmp(band_info, config->band + 2) != 0) {
        result = at_obj_exec_cmd(client, resp, "AT+QBAND=%s", config->band);
        if (result != RT_EOK) {
            LOG_E("nbiot set band result: %d", result);
            goto ERROR;
        }
        LOG_D("set band success");
    }

    char cipca_info[64] = {0};
    result = at_obj_exec_cmd(client, resp, "AT+CIPCA?");
    if (result != RT_EOK) {
        LOG_D("at_obj_exec_cmd AT+CIPCA? failed");
        goto ERROR;
    }
    if (at_resp_parse_line_args(resp, 2, "+CIPCA: %s", cipca_info) <= 0) {
        LOG_D("at_resp_parse_line_args AT+AT+CIPCA? failed");
        goto ERROR;
    }
    if (rt_strcmp(cipca_info, config->cipca) != 0) {
        result = at_obj_exec_cmd(client, resp, "AT+CIPCA=%s", config->cipca);
        if (result != RT_EOK) {
            LOG_E("nbiot set cipca result: %d", result);
            goto ERROR;
        }
        LOG_D("set cipca success");
    }

ERROR:
    at_delete_resp(resp);
    return result;
}

rt_err_t nbiot_lwm2m_register()
{
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_response_t resp = at_create_resp(128, 0, rt_tick_from_millisecond(3000));
    if (resp == RT_NULL) {
        LOG_E("create resp failed.");
        return RT_ERROR;
    }

    result = at_obj_exec_cmd(client, resp, "AT+QIOTREG=1");
    if (result != RT_EOK) {
        LOG_E("send qiotreg failed: %d", result);
        at_delete_resp(resp);
        return result;
    }
    LOG_D("send qiotreg=1 success");

    rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
    QIOT_SUBSCRIBE_EVENT_CODE = -1;
    rt_mutex_release(&qiot_event_mutex);
    for (int retry_times=0; retry_times < 10; retry_times++) {
        rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
        if (QIOT_SUBSCRIBE_EVENT_CODE == 10200) {
            rt_mutex_release(&qiot_event_mutex);
            at_delete_resp(resp);
            LOG_D("wait 3,10200 success, lwm2m has connect ready");
            return RT_EOK;
        }
        rt_mutex_release(&qiot_event_mutex);
        rt_thread_mdelay(5000);
    }

    LOG_D("wait 3,10200 failed, lwm2m connect failed");
    at_delete_resp(resp);
    return RT_ERROR;
}
MSH_CMD_EXPORT(nbiot_lwm2m_register, nbiot_lwm2m_register);

rt_err_t nbiot_lwm2m_deregister()
{
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_response_t resp = at_create_resp(128, 0, rt_tick_from_millisecond(3000));
    if (resp == RT_NULL) {
        LOG_E("create resp failed.");
        return RT_ERROR;
    }

    result = at_obj_exec_cmd(client, resp, "AT+QIOTREG=0");
    if (result != RT_EOK) {
        LOG_E("send qiotreg failed: %d", result);
        at_delete_resp(resp);
        return result;
    }
    LOG_D("send qiotreg=0 success");

    at_delete_resp(resp);
    return result;
}
MSH_CMD_EXPORT(nbiot_lwm2m_deregister, nbiot_lwm2m_deregister);

rt_err_t nbiot_check_qiotstate(int retry_times)
{
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_response_t resp = at_create_resp(128, 0, rt_tick_from_millisecond(3000));
    if (resp == RT_NULL) {
        LOG_E("create resp failed.");
        return RT_ERROR;
    }
    
    int state = -1;
    result = at_obj_exec_cmd(client, resp, "AT+QIOTSTATE?");
    if (result == RT_EOK) {
        if (at_resp_parse_line_args(resp, 2, "+QIOTSTATE: %d", &state) > 0) {
            LOG_D("get qiotstate state: %d", state);
            if (state == 8) {
                at_delete_resp(resp);
                return RT_EOK;
            }
            else {
                at_delete_resp(resp);
                return RT_ERROR;
            }
        }
    }
    at_delete_resp(resp);  // after at_resp_parse_line_args, delete resp will cause dump
    return RT_ERROR;
}
MSH_CMD_EXPORT(nbiot_check_qiotstate, nbiot_check_qiotstate);

rt_err_t nbiot_report_model_data(const char *data, rt_size_t length)
{
    LOG_D("report_model_data: %s; length: %d\n", data, length);
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_response_t resp = at_create_resp(128, 2, rt_tick_from_millisecond(3000));
    if (resp == RT_NULL) {
        LOG_E("create resp failed.");
        return RT_ERROR;
    }

    result = at_obj_exec_cmd(client, resp, "AT+QIOTMODELTD=100,%d", length);
    if (result != RT_EOK) {
        LOG_E("qiotmodeltd set failed: %d\n", result);
        at_delete_resp(resp);
        return RT_ERROR;
    }
    rt_thread_mdelay(500);  // After the > response, it is recommended for the MCU to wait for 500 ms before sending the data.

    rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
    QIOT_DATE_SEND_EVENT_CODE = -1;
    rt_mutex_release(&qiot_event_mutex);
    at_client_obj_send(client, data, length);
    at_client_obj_send(client, "\x1A", 1);

    for (int i=0; i < 30; i++) {
        rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
        if (QIOT_DATE_SEND_EVENT_CODE == -1) {
            rt_mutex_release(&qiot_event_mutex);
            rt_thread_mdelay(1000);
            continue;
        }
        if (QIOT_DATE_SEND_EVENT_CODE == 10210) {
            rt_mutex_release(&qiot_event_mutex);
            at_delete_resp(resp);
            return RT_EOK;
        }
        else {
            rt_mutex_release(&qiot_event_mutex);
            at_delete_resp(resp);
            return RT_ERROR;
        }
    }
    at_delete_resp(resp);
    return RT_ERROR;
}

rt_err_t nbiot_report_ctrl_data(const char *data, rt_size_t length)
{
    LOG_D("report_ctrl_data: %s; length: %d\n", data, length);
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_response_t resp = at_create_resp(128, 2, rt_tick_from_millisecond(5000));
    if (resp == RT_NULL) {
        LOG_E("create resp failed.");
        return RT_ERROR;
    }

    result = at_obj_exec_cmd(client, resp, "AT+QIOTMODELTD=100,%d", length);
    if (result != RT_EOK) {
        LOG_E("qiotmodeltd set failed: %d\n", result);
        at_delete_resp(resp);
        return RT_ERROR;
    }
    rt_thread_mdelay(500);  // After the > response, it is recommended for the MCU to wait for 500 ms before sending the data.

    rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
    QIOT_DATE_RECV_EVENT_CODE = -1;
    rt_mutex_release(&qiot_event_mutex);
    at_client_obj_send(client, data, length);
    at_client_obj_send(client, "\x1A", 1);

    for (int i=0; i < 30; i++) {
        rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
        if (QIOT_DATE_RECV_EVENT_CODE == -1) {
            rt_mutex_release(&qiot_event_mutex);
            rt_thread_mdelay(1000);
            continue;
        }
        if (QIOT_DATE_RECV_EVENT_CODE == 10210) {
            rt_mutex_release(&qiot_event_mutex);
            at_delete_resp(resp);
            return RT_EOK;
        }
        else {
            rt_mutex_release(&qiot_event_mutex);
            at_delete_resp(resp);
            return RT_ERROR;
        }
    }
    at_delete_resp(resp);
    return RT_ERROR;
}

rt_err_t nbiot_recv_ctrl_data(int req_length, struct ServerCtrlData *server_ctrl_data_ptr)
{
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_response_t resp = at_create_resp(128, 2, rt_tick_from_millisecond(3000));
    if (resp == RT_NULL) {
        LOG_E("create resp failed.");
        return RT_ERROR;
    }

    int retry_times = 0;
    int got_collect_interval_item_flag = 0;
    int got_cat1_upload_file_item = 0;
    while (1) {
        int cur_len = 0;
        int remain_len = 0;
        int remain_pieces = 0;

        result = at_obj_exec_cmd(client, resp, "AT+QIOTMODELRD=%d", req_length);
        if (result != RT_EOK) {
            LOG_E("qiotmodelrd set failed: %d\n", result);
            break;
        }

        int result = at_resp_parse_line_args(resp, 1, "+QIOTMODELRD: %d,%d,%d", &cur_len, &remain_len, &remain_pieces);
        LOG_D("at_resp_parse_line_args result: %d", result);
        if (result > 0) {
            LOG_D("cur_len: %d, remain_len: %d, remain_pieces: %d", cur_len, remain_len, remain_pieces);
            if (cur_len <= 0 && remain_pieces <= 0) {
                if (retry_times >= 30) {
                    break;
                }
            }
            const char *data = at_resp_get_line(resp, 2);
            LOG_D("recv ctrl data from server: %s", data);
            cJSON *root = cJSON_Parse(data);
            if (root != NULL) {
                LOG_D("cJSON: %s", cJSON_Print(root));
                cJSON *collect_interval_item = cJSON_GetObjectItem(root, "12");
                if (collect_interval_item != NULL) {
                    LOG_D("key: %s, value: %d", collect_interval_item->string, collect_interval_item->valueint);
                    server_ctrl_data_ptr->CollectInterval = collect_interval_item->valueint;
                    got_collect_interval_item_flag = 1;
                }
                cJSON *cat1_upload_file_item = cJSON_GetObjectItem(root, "13");
                if (cat1_upload_file_item != NULL) {
                    LOG_D("key: %s, value: %d", cat1_upload_file_item->string, cat1_upload_file_item->valueint);
                    server_ctrl_data_ptr->Cat1FileUpload = cat1_upload_file_item->valueint;
                    got_cat1_upload_file_item = 1;
                }
                cJSON_free(root);
            }
        }
        
        if (got_cat1_upload_file_item && got_collect_interval_item_flag) {
            LOG_D("got cat1_upload_file_item and collect_interval_item");
            break;
        }

        retry_times += 1;
        LOG_D("nbiot_recv_ctrl_data restry_times: %d", retry_times);
        rt_thread_mdelay(1000);
    }

    at_delete_resp(resp);
    return RT_ERROR;
}

rt_err_t nbiot_get_current_datetime(datetime_t dt)
{
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_response_t resp = at_create_resp(128, 1, rt_tick_from_millisecond(3000));
    if (resp == RT_NULL) {
        LOG_E("create resp failed.");
        return RT_ERROR;
    }

    result = at_obj_exec_cmd(client, resp, "AT+CCLK?");
    if (result != RT_EOK) {
        LOG_E("get current datetime failed: %d\n", result);
        at_delete_resp(resp);
        return result;
    }

    char data[64] = {0};
    if(rt_mq_recv(cclk_data_urc_queue, data, 64, rt_tick_from_millisecond(3000)) > 0) {
        LOG_D("recv cclk data: %s", data);
        sscanf(data, "+CCLK: %d/%d/%d,%d:%d:%d+%d", &(dt->year), &(dt->month), &(dt->day), &(dt->hour), &(dt->minute), &(dt->second), &(dt->zone));
        at_delete_resp(resp);
        return RT_EOK;
    }

    at_delete_resp(resp);
    return RT_ERROR;
}

rt_err_t nbiot_set_qiotlocext(char *nmea_string)
{
    rt_err_t result = RT_EOK;
    at_client_t client = RT_NULL;

    client = at_client_get(NBIOT_AT_UART_NAME);
    if (client == RT_NULL) {
        LOG_E("nbiot at client not inited!");
        return RT_ERROR;
    }

    at_response_t resp = at_create_resp(512, 1, rt_tick_from_millisecond(3000));
    if (resp == RT_NULL) {
        LOG_E("create resp failed.");
        return RT_ERROR;
    }

    rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
    QIOT_DATE_RECV_EVENT_CODE = -1;
    rt_mutex_release(&qiot_event_mutex);

    result = at_obj_exec_cmd(client, resp, "AT+QIOTLOCEXT=\"%s\"", nmea_string);
    LOG_D("AT+QIOTLOCEXT send nmea cmd result: %d", result);
    if (result == RT_EOK) {
        for (int i=0; i < 30; i++) {
            rt_mutex_take(&qiot_event_mutex, RT_WAITING_FOREVER);
            if (QIOT_DATE_RECV_EVENT_CODE == -1) {
                rt_mutex_release(&qiot_event_mutex);
                rt_thread_mdelay(1000);
                continue;
            }
            if (QIOT_DATE_RECV_EVENT_CODE == 10220) {
                rt_mutex_release(&qiot_event_mutex);
                at_delete_resp(resp);
                return RT_EOK;
            }
            else {
                rt_mutex_release(&qiot_event_mutex);
                at_delete_resp(resp);
                return RT_ERROR;
            }
            rt_thread_mdelay(1000);
        }
    }
    LOG_E("set qiotlocext result: %d\n", result);
    at_delete_resp(resp);
    return result;
}
